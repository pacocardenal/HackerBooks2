...
class BookCoreDataViewController: UIViewController {
    ...
    //MARK: - Syncing
    func syncViewWithModel(book: BookCoreData){
        
        //coverView.image = UIImage(data: _model._image.data)
        title = _model.title
        coverView.image = UIImage(data: (_model.photo?.data)! as Data)
        print("Title en book: \(title)")
